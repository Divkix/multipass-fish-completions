...
complete -c multipass -n "__fish_seen_subcommand_from find" -l format -d 'Output list in the requested format.Valid formats are: table (default), json, 
csv and yaml'
...
complete -c multipass -n "__fish_seen_subcommand_from get" -l raw -d 'Output in raw format. For now, this affects only the representation of empty 
...
complete -c multipass -n "__fish_seen_subcommand_from info" -l format -d 'Output info in the requested format. Valid formats are: table (default), json, 
csv and yaml'
...
complete -c multipass -n "__fish_seen_subcommand_from launch" -s m -l mem -d 'Amount of memory to allocate. Positive integers, in bytes, or with K, M, G 
suffix. Minimum: 128M, default: 1G.' -r
complete -c multipass -n "__fish_seen_subcommand_from launch" -s d -l disk -d 'Disk space to allocate. Positive integers, in bytes, or with K, M, G 
suffix. Minimum: 512M, default: 5G.' -r
complete -c multipass -n "__fish_seen_subcommand_from launch" -s n -l name -d 'Name for the instance. If it is primary (the configured primary instance 
name), the user home directory is mounted inside the newly launched instance, in Home.' -r
...
complete -c multipass -n "__fish_seen_subcommand_from list" -l format -d 'Output list in the requested format. Valid formats are: table (default), json, 
csv and yaml'
...
complete -c multipass -n "__fish_seen_subcommand_from restart" -d 'Names of instances to restart. If omitted, and without the --all option, primary will 
be assumed.' -r
...
complete -c multipass -n "__fish_seen_subcommand_from set" -d 'A key-value pair. The key specifies a path to the setting to configure. The value is its 
intended value.' -r
...
complete -c multipass -n "__fish_seen_subcommand_from shell" -d 'Name of the instance to open a shell on. If omitted, primary (the configured primary 
instance name) will be assumed. If the instance is not running, an attempt is made to start it (see `start` for more info).' -r
...
complete -c multipass -n "__fish_seen_subcommand_from start" -d 'Names of instances to start. If omitted, and without the --all option, primary (the 
configured primary instance name) will be assumed. If primary does not exist but is included in a successful start command (either implicitly or 
explicitly), it is launched automatically (see `launch` for more info).' -r
...
complete -c multipass -n "__fish_seen_subcommand_from stop" -d 'Names of instances to stop. If omitted, and without the --all option, primary will be 
assumed' -r
...
complete -c multipass -n "__fish_seen_subcommand_from suspend" -d 'Names of instances to suspend. If omitted, and without the --all option, primary will 
be assumed.' -r
...
complete -c multipass -n "__fish_seen_subcommand_from umount" -d 'Mount points, in <name>[:<path>] format, where <name> are instance names, and optional 
<path> are mount points. If omitted, all mounts will be removed from the named instances.' -r
